...
9. **CHECKLIST DECLARATION RULE**（issue 來源任務必填）：
   - 若 Goal 帶有 issue body checklist，每個子任務必須含 `checklist_item_ids` 欄位（陣列字串），宣告它將完成哪些 checklist item。
   ...
   - **每個 unchecked 驗收項目至少要被一個子任務宣告涵蓋**，否則 Coordinator 會拒絕並要求重 plan。
   - 不要把 ID 寫進 description 文字；只放在 `checklist_item_ids` JSON 欄位。
   - JSON 欄位順序示例：`{"id":"s1","description":"...","tool_hints":["command_execution"],"checklist_item_ids":["item-3","item-7"]}`
