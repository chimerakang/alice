...
type TelegramBot struct {
	...
	// UI prototype manager for /prototype command
...
func NewTelegramBot(config *Config, client Client) (*TelegramBot, error) {
	...
	if globalStorage != nil {
		pm, pmErr := NewPrototypeManager(client, filepath.Join(os.TempDir(), "alice-prototypes"), globalStorage)
		if pmErr != nil {
			log.Printf("[telegram] warning: prototype manager init failed: %v", pmErr)
		} else {
			bot.prototypeManager = pm
		}
	}
...
func (t *TelegramBot) handleMessage(key chatKey, userID int64, text string, caption string, photo []PhotoSize, voice *Voice, document *Document, mediaGroupID string, messageID int, replyToMessageID int) {
	...
	// 回覆截圖訊息 → 自動視為原型修改請求
	if replyToMessageID != 0 && t.prototypeManager != nil && !strings.HasPrefix(text, "/") {
		if proto, lookupErr := t.prototypeManager.storage.GetPrototypeByMessageID(int64(replyToMessageID)); lookupErr == nil {
			go func() {
				done := globalJobTracker.Start("telegram.prototype.edit_reply")
				defer done(nil)
				t.handlePrototypeReplyEdit(key, proto.ID, text)
			}()
			return
		}
	}
...
func (t *TelegramBot) handleCommand(key chatKey, text string) {
	...
	case "/prototype":
		t.handlePrototypeCommand(key, parts)

	case "/prototype-edit":
		t.handlePrototypeEditCommand(key, parts)

	case "/prototype-list":
		t.handlePrototypeListCommand(key)

	case "/prototype-export":
		t.handlePrototypeExportCommand(key, parts)
...
func (t *TelegramBot) handleCallbackQuery(key chatKey, userID int64, queryID, data string) {
	...
	case strings.HasPrefix(data, "proto:"):
		t.handlePrototypeCallback(key, queryID, data)
...
func (t *TelegramBot) handlePrototypeCommand(key chatKey, parts []string) {
	if t.prototypeManager == nil {
		t.send(key, t.getLocalizedMessage(key.chatID, "prototype_not_initialized", nil))
		return
	}
	if len(parts) < 2 {
		t.send(key, t.getLocalizedMessage(key.chatID, "prototype_generate_usage", nil))
		return
	}
	prompt := strings.Join(parts[1:], " ")
	t.send(key, t.getLocalizedMessage(key.chatID, "prototype_generating", nil))
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Minute)
	defer cancel()

	proto, err := t.prototypeManager.Generate(ctx, key.chatID, prompt)
	if err != nil {
		t.send(key, t.getLocalizedMessage(key.chatID, "prototype_generate_failed", map[string]string{"error": err.Error()}))
		return
	}

	pngBytes, err := t.prototypeManager.Screenshot(ctx, proto.CurrentHTML)
	if err != nil {
		t.send(key, t.getLocalizedMessage(key.chatID, "prototype_screenshot_failed", map[string]string{"id": shortPrototypeID(proto.ID), "error": err.Error()}))
		return
	}

	msgID, err := t.sendPhotoWithKeyboard(key, pngBytes, t.getLocalizedMessage(key.chatID, "prototype_caption", map[string]string{"version": fmt.Sprintf("%d", proto.Version), "id": shortPrototypeID(proto.ID)}), t.buildPrototypeKeyboard(key.chatID, proto.ID))
	if err != nil {
		t.send(key, t.getLocalizedMessage(key.chatID, "prototype_send_failed", map[string]string{"error": err.Error()}))
		return
	}

	proto.MessageID = int64(msgID)
	_ = t.prototypeManager.storage.UpdatePrototype(*proto)
}

func (t *TelegramBot) handlePrototypeEditCommand(key chatKey, parts []string) {
	if t.prototypeManager == nil {
		t.send(key, t.getLocalizedMessage(key.chatID, "prototype_not_initialized", nil))
		return
	}
	if len(parts) < 3 {
		t.send(key, t.getLocalizedMessage(key.chatID, "prototype_edit_usage", nil))
		return
	}
	protoID := parts[1]
	editPrompt := strings.Join(parts[2:], " ")
	t.send(key, t.getLocalizedMessage(key.chatID, "prototype_editing", nil))
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Minute)
	defer cancel()
	t.applyPrototypeEdit(ctx, key, protoID, editPrompt)
}

func (t *TelegramBot) handlePrototypeListCommand(key chatKey) {
	if t.prototypeManager == nil {
		t.send(key, t.getLocalizedMessage(key.chatID, "prototype_not_initialized", nil))
		return
	}
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	list, err := t.prototypeManager.List(ctx, key.chatID)
	if err != nil {
		t.send(key, t.getLocalizedMessage(key.chatID, "prototype_generate_failed", map[string]string{"error": err.Error()}))
		return
	}
	if len(list) == 0 {
		t.send(key, t.getLocalizedMessage(key.chatID, "prototype_list_empty", nil))
		return
	}
	msg := t.getLocalizedMessage(key.chatID, "prototype_list_header", map[string]string{"count": fmt.Sprintf("%d", len(list))})
	for _, p := range list {
		msg += fmt.Sprintf("• `%s` — v%d｜%s\n  _%s_\n\n", shortPrototypeID(p.ID), p.Version, p.UpdatedAt.Format("01/02 15:04"), p.OriginalPrompt)
	}
	msg += t.getLocalizedMessage(key.chatID, "prototype_list_footer", nil)
	t.sendMarkdown(key, msg)
}

func (t *TelegramBot) handlePrototypeExportCommand(key chatKey, parts []string) {
	if t.prototypeManager == nil {
		t.send(key, t.getLocalizedMessage(key.chatID, "prototype_not_initialized", nil))
		return
	}
	if len(parts) < 2 {
		t.send(key, t.getLocalizedMessage(key.chatID, "prototype_export_usage", nil))
		return
	}
	protoID := parts[1]
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	outPath, err := t.prototypeManager.Export(ctx, protoID)
	if err != nil {
		t.send(key, t.getLocalizedMessage(key.chatID, "prototype_export_failed", map[string]string{"error": err.Error()}))
		return
	}
	defer os.Remove(outPath)
	if err := t.sendDocument(key, outPath, t.getLocalizedMessage(key.chatID, "prototype_export_file_caption", nil)); err != nil {
		t.send(key, t.getLocalizedMessage(key.chatID, "prototype_send_file_failed", map[string]string{"error": err.Error()}))
	}
}

func (t *TelegramBot) handlePrototypeReplyEdit(key chatKey, protoID, editPrompt string) {
	t.send(key, t.getLocalizedMessage(key.chatID, "prototype_editing", nil))
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Minute)
	defer cancel()
	t.applyPrototypeEdit(ctx, key, protoID, editPrompt)
...
func (t *TelegramBot) applyPrototypeEdit(ctx context.Context, key chatKey, protoID, editPrompt string) {
	proto, err := t.prototypeManager.Edit(ctx, protoID, editPrompt)
	if err != nil {
		t.send(key, t.getLocalizedMessage(key.chatID, "prototype_edit_failed", map[string]string{"error": err.Error()}))
		return
	}
	pngBytes, err := t.prototypeManager.Screenshot(ctx, proto.CurrentHTML)
	if err != nil {
		t.send(key, t.getLocalizedMessage(key.chatID, "prototype_screenshot_edit_failed", map[string]string{"version": fmt.Sprintf("%d", proto.Version), "error": err.Error()}))
		return
	}
	msgID, err := t.sendPhotoWithKeyboard(key, pngBytes, t.getLocalizedMessage(key.chatID, "prototype_caption", map[string]string{"version": fmt.Sprintf("%d", proto.Version), "id": shortPrototypeID(proto.ID)}), t.buildPrototypeKeyboard(key.chatID, proto.ID))
	if err != nil {
...
func (t *TelegramBot) handlePrototypeCallback(key chatKey, queryID, data string) {
	// data format: "proto:<action>:<protoID>"  or  "proto:<action>:<protoID>:<sub>"
	parts := strings.SplitN(data, ":", 4)
	if len(parts) < 3 {
		t.answerCallbackQuery(queryID, t.getLocalizedMessage(key.chatID, "prototype_callback_invalid", nil))
		return
	}
	action := parts[1]
	protoID := parts[2]
	...
	if t.prototypeManager == nil {
		t.answerCallbackQuery(queryID, t.getLocalizedMessage(key.chatID, "prototype_callback_not_initialized", nil))
		return
	}

	switch action {
	case "color":
		t.answerCallbackQuery(queryID, t.getLocalizedMessage(key.chatID, "prototype_callback_color", nil))
		t.sendWithKeyboard(key, t.getLocalizedMessage(key.chatID, "prototype_color_menu_title", nil), t.buildColorSubMenu(key.chatID, protoID))
	case "layout":
		t.answerCallbackQuery(queryID, t.getLocalizedMessage(key.chatID, "prototype_callback_layout", nil))
		t.sendWithKeyboard(key, t.getLocalizedMessage(key.chatID, "prototype_layout_menu_title", nil), t.buildLayoutSubMenu(key.chatID, protoID))
	case "text":
		t.answerCallbackQuery(queryID, t.getLocalizedMessage(key.chatID, "prototype_callback_text", nil))
		t.sendWithKeyboard(key, t.getLocalizedMessage(key.chatID, "prototype_text_menu_title", nil), t.buildTextSubMenu(key.chatID, protoID))
	case "add":
		t.answerCallbackQuery(queryID, t.getLocalizedMessage(key.chatID, "prototype_callback_add", nil))
		t.sendWithKeyboard(key, t.getLocalizedMessage(key.chatID, "prototype_add_menu_title", nil), t.buildAddElementSubMenu(key.chatID, protoID))
	case "export":
		t.answerCallbackQuery(queryID, t.getLocalizedMessage(key.chatID, "prototype_callback_exporting", nil))
		go func() {
			done := globalJobTracker.Start("telegram.prototype.export")
			defer done(nil)
			t.handlePrototypeExportCommand(key, []string{"/prototype-export", protoID})
		}()
	case "redo":
		t.answerCallbackQuery(queryID, t.getLocalizedMessage(key.chatID, "prototype_callback_redoing", nil))
		go func() {
			done := globalJobTracker.Start("telegram.prototype.redo")
			defer done(nil)
			proto, err := t.prototypeManager.storage.GetPrototype(protoID)
			if err != nil {
				t.send(key, t.getLocalizedMessage(key.chatID, "prototype_not_found", map[string]string{"id": shortPrototypeID(protoID)}))
				return
			}
			ctx, cancel := context.WithTimeout(context.Background(), 3*time.Minute)
			defer cancel()
			newProto, err := t.prototypeManager.Generate(ctx, key.chatID, proto.OriginalPrompt)
			if err != nil {
				t.send(key, t.getLocalizedMessage(key.chatID, "prototype_redo_failed", map[string]string{"error": err.Error()}))
				return
			}
			pngBytes, err := t.prototypeManager.Screenshot(ctx, newProto.CurrentHTML)
			if err != nil {
				t.send(key, t.getLocalizedMessage(key.chatID, "prototype_screenshot_failed", map[string]string{"id": shortPrototypeID(newProto.ID), "error": err.Error()}))
				return
			}
			msgID, err := t.sendPhotoWithKeyboard(key, pngBytes, t.getLocalizedMessage(key.chatID, "prototype_redo_caption", map[string]string{"version": fmt.Sprintf("%d", newProto.Version), "id": shortPrototypeID(newProto.ID)}), t.buildPrototypeKeyboard(key.chatID, newProto.ID))
			if err == nil {
				newProto.MessageID = int64(msgID)
				_ = t.prototypeManager.storage.UpdatePrototype(*newProto)
			}
		}()
	case "apply":
		// sub-menu apply: parts[3] is a short code, kept under Telegram's
		// 64-byte callback_data limit even with a full UUID prototype ID.
		if len(parts) < 4 {
			t.answerCallbackQuery(queryID, t.getLocalizedMessage(key.chatID, "prototype_callback_invalid", nil))
			return
		}
		editDesc, ok := prototypeQuickEditDescription(parts[3])
		if !ok {
			t.answerCallbackQuery(queryID, t.getLocalizedMessage(key.chatID, "prototype_callback_invalid", nil))
			return
		}
		t.answerCallbackQuery(queryID, t.getLocalizedMessage(key.chatID, "prototype_callback_applying", nil))
		go func() {
			done := globalJobTracker.Start("telegram.prototype.apply")
			defer done(nil)
			ctx, cancel := context.WithTimeout(context.Background(), 3*time.Minute)
			defer cancel()
			t.applyPrototypeEdit(ctx, key, protoID, editDesc)
		}()
	case "back":
		t.answerCallbackQuery(queryID, t.getLocalizedMessage(key.chatID, "prototype_callback_back", nil))
		proto, err := t.prototypeManager.storage.GetPrototype(protoID)
		if err != nil {
			t.send(key, t.getLocalizedMessage(key.chatID, "prototype_not_found", map[string]string{"id": shortPrototypeID(protoID)}))
			return
		}
		t.sendWithKeyboard(key, t.getLocalizedMessage(key.chatID, "prototype_caption", map[string]string{"version": fmt.Sprintf("%d", proto.Version), "id": shortPrototypeID(proto.ID)}), t.buildPrototypeKeyboard(key.chatID, protoID))
	default:
		t.answerCallbackQuery(queryID, t.getLocalizedMessage(key.chatID, "prototype_callback_invalid", nil))
	}
}

func prototypeQuickEditDescription(code string) (string, bool) {
	switch strings.TrimSpace(code) {
	case "dark":
		return "將整體主題改成深色模式", true
	case "light":
		return "將整體主題改成淺色模式", true
	case "blue":
		return "將主色調改成藍色系", true
	case "green":
		return "將主色調改成綠色系", true
	case "purple":
		return "將主色調改成紫色系", true
	case "l1":
		return "將佈局改成單欄版面", true
	case "l2":
		return "將佈局改成雙欄版面", true
	case "l3":
		return "將佈局改成三欄版面", true
	case "cards":
		return "將佈局改成卡片網格版面", true
	case "list":
		return "將佈局改成列表版面", true
	case "tbigger":
		return "將標題字體放大", true
	case "tsmaller":
		return "將整體字體縮小一級", true
	case "tbold":
		return "將所有標題改為粗體", true
	case "tzh":
		return "將所有文字改成繁體中文", true
	case "ten":
		return "將所有文字改成英文", true
	case "img":
		return "在適當位置加入一個圖片展示區", true
	case "btn":
		return "在適當位置加入一個 CTA 按鈕", true
	case "table":
		return "加入一個資料表格", true
	case "feature":
		return "加入三個特色功能卡片", true
	case "nav":
		return "加入頂部導航列", true
	default:
		return "", false
...
func (t *TelegramBot) buildPrototypeKeyboard(chatID int64, protoID string) map[string]interface{} {
	return map[string]interface{}{
		"inline_keyboard": []interface{}{
			[]interface{}{
				map[string]string{"text": t.getLocalizedMessage(chatID, "prototype_btn_color", nil), "callback_data": "proto:color:" + protoID},
				map[string]string{"text": t.getLocalizedMessage(chatID, "prototype_btn_layout", nil), "callback_data": "proto:layout:" + protoID},
				map[string]string{"text": t.getLocalizedMessage(chatID, "prototype_btn_text", nil), "callback_data": "proto:text:" + protoID},
			},
			[]interface{}{
				map[string]string{"text": t.getLocalizedMessage(chatID, "prototype_btn_add", nil), "callback_data": "proto:add:" + protoID},
				map[string]string{"text": t.getLocalizedMessage(chatID, "prototype_btn_export", nil), "callback_data": "proto:export:" + protoID},
				map[string]string{"text": t.getLocalizedMessage(chatID, "prototype_btn_redo", nil), "callback_data": "proto:redo:" + protoID},
			},
		},
	}
}

func (t *TelegramBot) buildColorSubMenu(chatID int64, protoID string) map[string]interface{} {
	return map[string]interface{}{
		"inline_keyboard": []interface{}{
			[]interface{}{
				map[string]string{"text": t.getLocalizedMessage(chatID, "prototype_btn_dark", nil), "callback_data": "proto:apply:" + protoID + ":dark"},
				map[string]string{"text": t.getLocalizedMessage(chatID, "prototype_btn_light", nil), "callback_data": "proto:apply:" + protoID + ":light"},
				map[string]string{"text": t.getLocalizedMessage(chatID, "prototype_btn_blue", nil), "callback_data": "proto:apply:" + protoID + ":blue"},
			},
			[]interface{}{
				map[string]string{"text": t.getLocalizedMessage(chatID, "prototype_btn_green", nil), "callback_data": "proto:apply:" + protoID + ":green"},
				map[string]string{"text": t.getLocalizedMessage(chatID, "prototype_btn_purple", nil), "callback_data": "proto:apply:" + protoID + ":purple"},
				map[string]string{"text": t.getLocalizedMessage(chatID, "prototype_btn_back", nil), "callback_data": "proto:back:" + protoID},
			},
		},
	}
}

func (t *TelegramBot) buildLayoutSubMenu(chatID int64, protoID string) map[string]interface{} {
	return map[string]interface{}{
		"inline_keyboard": []interface{}{
			[]interface{}{
				map[string]string{"text": t.getLocalizedMessage(chatID, "prototype_btn_layout_1col", nil), "callback_data": "proto:apply:" + protoID + ":l1"},
				map[string]string{"text": t.getLocalizedMessage(chatID, "prototype_btn_layout_2col", nil), "callback_data": "proto:apply:" + protoID + ":l2"},
				map[string]string{"text": t.getLocalizedMessage(chatID, "prototype_btn_layout_3col", nil), "callback_data": "proto:apply:" + protoID + ":l3"},
			},
			[]interface{}{
				map[string]string{"text": t.getLocalizedMessage(chatID, "prototype_btn_layout_card", nil), "callback_data": "proto:apply:" + protoID + ":cards"},
				map[string]string{"text": t.getLocalizedMessage(chatID, "prototype_btn_layout_list", nil), "callback_data": "proto:apply:" + protoID + ":list"},
				map[string]string{"text": t.getLocalizedMessage(chatID, "prototype_btn_back", nil), "callback_data": "proto:back:" + protoID},
			},
		},
	}
...
func (t *TelegramBot) buildTextSubMenu(chatID int64, protoID string) map[string]interface{} {
	return map[string]interface{}{
		"inline_keyboard": []interface{}{
			[]interface{}{
				map[string]string{"text": t.getLocalizedMessage(chatID, "prototype_btn_text_bigger", nil), "callback_data": "proto:apply:" + protoID + ":tbigger"},
				map[string]string{"text": t.getLocalizedMessage(chatID, "prototype_btn_text_smaller", nil), "callback_data": "proto:apply:" + protoID + ":tsmaller"},
				map[string]string{"text": t.getLocalizedMessage(chatID, "prototype_btn_text_bold", nil), "callback_data": "proto:apply:" + protoID + ":tbold"},
			},
			[]interface{}{
				map[string]string{"text": t.getLocalizedMessage(chatID, "prototype_btn_text_zh", nil), "callback_data": "proto:apply:" + protoID + ":tzh"},
				map[string]string{"text": t.getLocalizedMessage(chatID, "prototype_btn_text_en", nil), "callback_data": "proto:apply:" + protoID + ":ten"},
				map[string]string{"text": t.getLocalizedMessage(chatID, "prototype_btn_back", nil), "callback_data": "proto:back:" + protoID},
			},
		},
	}
...
func (t *TelegramBot) buildAddElementSubMenu(chatID int64, protoID string) map[string]interface{} {
	return map[string]interface{}{
		"inline_keyboard": []interface{}{
			[]interface{}{
				map[string]string{"text": t.getLocalizedMessage(chatID, "prototype_btn_add_image", nil), "callback_data": "proto:apply:" + protoID + ":img"},
				map[string]string{"text": t.getLocalizedMessage(chatID, "prototype_btn_add_button", nil), "callback_data": "proto:apply:" + protoID + ":btn"},
				map[string]string{"text": t.getLocalizedMessage(chatID, "prototype_btn_add_table", nil), "callback_data": "proto:apply:" + protoID + ":table"},
			},
			[]interface{}{
				map[string]string{"text": t.getLocalizedMessage(chatID, "prototype_btn_add_card", nil), "callback_data": "proto:apply:" + protoID + ":feature"},
				map[string]string{"text": t.getLocalizedMessage(chatID, "prototype_btn_add_nav", nil), "callback_data": "proto:apply:" + protoID + ":nav"},
				map[string]string{"text": t.getLocalizedMessage(chatID, "prototype_btn_back", nil), "callback_data": "proto:back:" + protoID},
			},
		},
	}
...
// sendPhotoWithKeyboard 發送帶 inline keyboard 的圖片，返回訊息 ID
func (t *TelegramBot) sendPhotoWithKeyboard(key chatKey, pngBytes []byte, caption string, keyboard map[string]interface{}) (int, error) {
	// 寫到臨時檔案
	tmp, err := os.CreateTemp("", "proto-screenshot-*.png")
	if err != nil {
		return 0, fmt.Errorf("sendPhotoWithKeyboard: tempfile: %w", err)
	}
	tmpPath := tmp.Name()
	defer os.Remove(tmpPath)
	if _, err := tmp.Write(pngBytes); err != nil {
		tmp.Close()
		return 0, fmt.Errorf("sendPhotoWithKeyboard: write: %w", err)
	}
	tmp.Close()

	file, err := os.Open(tmpPath)
	if err != nil {
		return 0, fmt.Errorf("sendPhotoWithKeyboard: open: %w", err)
	}
	defer file.Close()

	var b bytes.Buffer
	writer := multipart.NewWriter(&b)
	fw, err := writer.CreateFormFile("photo", "screenshot.png")
	if err != nil {
		return 0, err
	}
	...
	if err != nil {
		return 0, fmt.Errorf("sendPhotoWithKeyboard: marshal keyboard: %w", err)
	}
	writer.WriteField("reply_markup", string(kbJSON))
	contentType := writer.FormDataContentType()
	writer.Close()

	apiURL := fmt.Sprintf("https://api.telegram.org/bot%s/sendPhoto", t.config.TelegramToken)
	req, err := http.NewRequest("POST", apiURL, &b)
	if err != nil {
		return 0, err
	}
	...
	if res, ok := result["result"].(map[string]interface{}); ok {
		if msgID, ok := res["message_id"].(float64); ok {
...
// sendWithKeyboard 發送帶 inline keyboard 的文字訊息
func (t *TelegramBot) sendWithKeyboard(key chatKey, text string, keyboard map[string]interface{}) {
	params := map[string]interface{}{
		"chat_id":      key.chatID,
		"text":         text,
		"parse_mode":   "Markdown",
	...
	_, _ = t.sendMessageSync("sendMessage", params)
}
