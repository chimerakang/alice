...
| # | Task | Issue | Status |
|---|------|-------|--------|
| P13.1 | **🐛 Smart Routing 導致對話上下文丟失：Model 切換時強制清空 Session** | [#79](https://github.com/chimerakang/alice/issues/79) | ✅ |
| | — 移除強制清空 sessionID 邏輯 | | ✅ |
| | — 改進決策日誌以記錄同一 session 內的 model 切換 | | ✅ |
| | — 改進成本計算，分別追蹤每個 model 的 token 使用 | | ✅ |
| | — 測試跨 model session 的對話連貫性 | | ✅ |
| | — 驗證成本計算的準確性 | | ✅ |
| P13.2 | **Add project_path field to performance_metrics table for per-project token cost tracking** | [#78](https://github.com/chimerakang/alice/issues/78) | 🔄 |
...
| P13.3 | **/usage 指令增加按模型分類的 token 用量與費用顯示** | [#77](https://github.com/chimerakang/alice/issues/77) | ✅ |
...
| P13.4 | **Bot 多國語系支援 — 可切換顯示語言** | [#76](https://github.com/chimerakang/alice/issues/76) | 🔄 |
...
| | — 撰寫新增語系的開發指南文件 | | ☐ |
| P13.5 | **回填歷史資料缺失的 model 與費用欄位** | [#75](https://github.com/chimerakang/alice/issues/75) | ✅ |
...
| P13.6 | **P13: Savings Calculator - 智慧路由省錢可視化（商業賣點）** | [#74](https://github.com/chimerakang/alice/issues/74) | 🔄 |
...
| | — Dashboard 頁面整合 | | ☐ |
| | — Performance 頁面整合 | | ☐ |
| P13.7 | **P13: Per-Model Cost Tracking - 按模型記錄 Token 成本與用量** | [#73](https://github.com/chimerakang/alice/issues/73) | ✅ |
...
| P13.8 | **P13: Dynamic Model Routing - 智慧模型路由降低 Token 成本** | [#72](https://github.com/chimerakang/alice/issues/72) | ✅ |
...
| P13.9 | **🐛 Telegram /tasks 無法顯示 GitHub Issues（private repo 認證失敗）** | [#71](https://github.com/chimerakang/alice/issues/71) | ✅ |
| P13.10 | **Security 頁面：Events Trend 圖表未跟隨時間篩選器 + 標題硬編碼** | [#70](https://github.com/chimerakang/alice/issues/70) | 🔄 |
| P13.11 | **Security 頁面：PII Detection Records 缺乏上下文資訊，無法判斷問題內容** | [#69](https://github.com/chimerakang/alice/issues/69) | 🔄 |
| P13.12 | **🔍 智能 Token 檢測引擎 - SecureGuard 核心功能** | [#61](https://github.com/chimerakang/alice/issues/61) | 🔄 |
| | — **常見 Token 模式庫** | | ☐ |
...
| | — 用戶行為分析 | | ☐ |
| | — 系統健康度監控 | | ☐ |
| P13.13 | **💰【商業功能】Alice SecureGuard - AI 開發防洩密系統** | [#60](https://github.com/chimerakang/alice/issues/60) | 🔄 |
| | — **Token 模式識別引擎** | | ☐ |
...
| | — 成為行業標準解決方案 | | ☐ |
| | — 被主要分析師機構 (Gartner) 認可 | | ☐ |
| P13.14 | **🚨 智能異常檢測系統 - AI 使用模式風險預警** | [#59](https://github.com/chimerakang/alice/issues/59) | 🔄 |
| | — **成本異常** | | ☐ |
...
| | — 提供具體的處理建議 | | ☐ |
| | — 支援快速確認和關閉 | | ☐ |
| P13.15 | **📊 自動週報生成系統 - 團隊 AI 使用效益報告** | [#57](https://github.com/chimerakang/alice/issues/57) | 🔄 |
...
| | — 警告和建議具體可行 | | ☐ |
| | — 報告易於理解和分享 | | ☐ |
| P13.16 | **🎨 Manager Dashboard 前端介面 - 主管視角的 AI 使用分析頁面** | [#55](https://github.com/chimerakang/alice/issues/55) | 🔄 |
...
| | — 數據呈現清晰易懂 | | ☐ |
| | — 警告和異常能快速識別 | | ☐ |
| P13.17 | **🔧 擴展 PerformanceMetrics - 增加管理層洞察數據收集** | [#52](https://github.com/chimerakang/alice/issues/52) | 🔄 |
...
| | — API 能返回團隊使用模式分析數據 | | ☐ |
| | — 任務成功率判斷準確率 > 80% | | ☐ |
| P13.18 | **【商業功能】單機版主管報告系統 - AI 使用效益監控** | [#50](https://github.com/chimerakang/alice/issues/50) | 🔄 |
| | — **團隊 AI 使用健康度指標** | | ☐ |
| | — 個人效益排行榜（誰用AI效果好/差） | | ☐ |
| | — Bug率變化趨勢（AI輔助前後對比） | | ☐ |
| | — 開發效率提升統計 | | ☐ |
...
| | — 週報開信率 > 60% | | ☐ |
| | — 功能使用率 > 70% | | ☐ |
| P13.19 | **【MVP】AI 開發審計系統 - 企業安全合規功能** | [#48](https://github.com/chimerakang/alice/issues/48) | 🔄 |
| | — **完整決策記錄追蹤** | | ☐ |
| | — 記錄每個 AI 決策的完整上下文 | | ☐ |
...
| | — 驗證技術架構可擴展性 | | ☐ |
| | — 確認商業模式可行性 | | ☐ |
| P13.20 | **🐛 Telegram 訊息 UTF-8 編碼錯誤導致發送失敗** | [#39](https://github.com/chimerakang/alice/issues/39) | ✅ |
| P13.21 | **🐛 Telegram 429 Rate Limiting: 多 Agent 同時發送導致訊息遺失** | [#38](https://github.com/chimerakang/alice/issues/38) | ✅ |

## P14 - Commercialization Strategy (🔄 0%)

| # | Task | Issue | Status |
|---|------|-------|--------|
| P14.1 | **Alice 商業化執行藍圖：6個月行動計畫** | [#58](https://github.com/chimerakang/alice/issues/58) | 🔄 |
| | — 實現基礎授權驗證系統 | | ☐ |
| | — 添加使用量追蹤與計費功能 | | ☐ |
| | — 優化 Dashboard 企業級展示 | | ☐ |
| | — 改善產品穩定性與效能 | | ☐ |
...
| | — 客戶付款延遲：建立付款保障機制 | | ☐ |
| | — 投資回報不佳：設定止損線 | | ☐ |
| P14.2 | **品牌定位與行銷策略規劃** | [#56](https://github.com/chimerakang/alice/issues/56) | 🔄 |
...
| | — 負面評論管理機制 | | ☐ |
| | — 品牌形象危機處理預案 | | ☐ |
| P14.3 | **產品授權與定價模式設計** | [#54](https://github.com/chimerakang/alice/issues/54) | 🔄 |
| | — 單機授權：$2,999/年 | | ☐ |
...
| | — 跨國企業授權模式設計 | | ☐ |
| | — 稅務與會計處理方案 | | ☐ |
| P14.4 | **競品分析深化：Entire Checkpoints vs Alice 差異化策略** | [#53](https://github.com/chimerakang/alice/issues/53) | 🔄 |
...
| | — 競爭對手動態監控流程 | | ☐ |
| | — 市場反應收集與分析 | | ☐ |
| P14.5 | **多人版架構設計：從單機到團隊協作** | [#51](https://github.com/chimerakang/alice/issues/51) | 🔄 |
...
| | — 資料遷移風險控制 | | ☐ |
| | — 效能影響評估 | | ☐ |
| P14.6 | **Alice 商業化：單機版隱私優先定位策略** | [#49](https://github.com/chimerakang/alice/issues/49) | 🔄 |
...
| # | Task | Issue | Status |
|---|------|-------|--------|
| P8.5.1 | **🔄 Topic-Project 對應持久化 — 重啟後保留設定** | [#33](https://github.com/chimerakang/alice/issues/33) | ✅ |
| | — `/project` 設定後重啟 bot，topic 仍記得之前的 project 路徑 | | ✅ |
| | — 新 topic（從未設定過）仍使用預設路徑 | | ✅ |
| | — 多個 topic 各自獨立持久化 | | ✅ |
| P8.5.2 | **📋 /tasks 指令 — Telegram 查看待辦工作清單** | [#31](https://github.com/chimerakang/alice/issues/31) | ✅ |
...
| # | Task | Issue | Status |
|---|------|-------|--------|
| P9.5.1 | **🔧 修復跨專案圖片存取問題 — 圖片路徑權限修復** | [#35](https://github.com/chimerakang/alice/issues/35) | ✅ |
| | — **圖片複製機制** - 將圖片從 Alice 臨時目錄複製到目標專案 | | ✅ |
| | — **相對路徑使用** - 使用 `temp/photo.jpg` 而非絕對路徑 | | ✅ |
| | — **專案級清理** - Agent 結束時清理專案臨時檔案 | | ✅ |
| | — **目錄確保存在** - 自動建立 `{projectDir}/temp/` 目錄 | | ✅ |
| | — **錯誤處理** - 複製失敗時的適當錯誤訊息 | | ✅ |
| P9.5.2 | **🖼️ 多張圖片批次處理支援 — Telegram 媒體群組分析** | [#34](https://github.com/chimerakang/alice/issues/34) | ✅ |
